...
<!--/顶部菜单-->
    ...
    <div class="">
        ...
        <div class="row">
            ...
            <div class="col-md-3 col-xs-12">
                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <div id="tag_add_form" class="form-horizontal form-label-left input_mask">

                            <div class="form-group">
                                <div class="col-md-12 col-sm-12 col-xs-12">
                                    <input type="text" class="form-control" placeholder="标签名称" name="tag_name">
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-md-12 col-sm-12 col-xs-12">
                                    <input type="text" class="form-control" placeholder="图片地址" name="tag_img">
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-md-12 col-sm-12 col-xs-12">
                                    <textarea placeholder="标签描述" class="form-control" name="tag_description"></textarea>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="col-md-12 col-sm-12 col-xs-12">
                                    <button type="submit" class="btn btn-success" onclick="tag_Add()">添加</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <!--修改表单模态框-->
        <div class="modal fade tag-modify-form-modal" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="" class="form-horizontal form-label-left input_mask">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12 form-group" style="display: none">
                                <label>标签ID</label>
                                <input type="text" class="form-control " placeholder="标签ID" name="tag_id">
                            </div>
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12 form-group">
                                <label>标签图片地址</label>
                                <input type="text" class="form-control " placeholder="设置访问密码" name="tag_img">
                            </div>
                            <div class="col-md-12 col-sm-12 col-xs-12 form-group">
                                <label>标签描述</label>
                                <textarea placeholder="标签描述" class="form-control" name="tag_description"></textarea>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" onclick="tag_Edit()">保存</button>
                    </div>
...
<script src="/yhq_blog/Template/Admin/Static/js/custom.js"></script>
    ...
    <script type="text/javascript">
        ...
        function init_DataTables() {
            ...
            //初始化文章列表
            tag_table = $('#datatable-checkbox').dataTable({
                "sScrollX": "100%",
...
//            lengthMenu: [5, 10, 25, 50],
                ...
                ajax: { // 获取数据
                    url: "<?php echo U('blog_tag/getList');?>",
                    type: "POST",
                ...
                columns: [
                    ...
                    {title: "id", name: 'tag_id', data: "tag_id"},
                    {title: "名称", name: 'tag_name', data: "tag_name", defaultContent: "默认值"},
                    {title: "图片", name: 'tag_img', data: "tag_img"},
                    {title: "描述", name: 'tag_description', data: "tag_description"},
                    {title: "计数", name: 'tag_count', data: "tag_count"},
                ...
                "columnDefs": [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            var html = "<button type='button' class='btn btn-primary' data-toggle='modal' data-target='.tag-modify-form-modal' onclick='tag_OpenModifyForm(" + row.tag_id + ")'>编辑</button>" +
                                "   <button name='btn-del' class='btn btn-danger' onclick='tag_Del(" + row.tag_id + ")'>删除</button>";
                            return html;
                        }
                    },
                    {
...
//                    }
        ...
        function init_BootstrapModal() {
            $('.tag-modify-form-modal').modal({
                keyboard: true,
                show: false
            })
            //关闭窗口时，将表单数据清空
            $('.tag-modify-form-modal').on('hidden.bs.modal', function () {
                $(".tag-modify-form-modal input[name='tag_id']").val('');
                $(".tag-modify-form-modal input[name='tag_name']").val('');
                $(".tag-modify-form-modal input[name='tag_img']").val('');
                $(".tag-modify-form-modal textarea[name='tag_description']").val('');
                $(".tag-modify-form-modal select[name='tag_parent_id']").val('0');
                $(".tag-modify-form-modal select[name='tag_parent_id'] option[disabled='disabled']").removeAttr('disabled');
        ...
        function tag_Add() {
            ...
            var jsonData = {
                "tag_name": $("#tag_add_form input[name='tag_name']").val(),
                "tag_img": $("#tag_add_form input[name='tag_img']").val(),
                "tag_description": $("#tag_add_form textarea[name='tag_description']").val(),
                "tag_parent_id": $("#tag_add_form select[name='tag_parent_id']").val()
            };
            $.ajax({
                type: 'post',
                url: "<?php echo U('blog_tag/add');?>",
                data: jsonData,
                dataType: "json",
                success: function (msg) {
                    if (msg.status == true) {
                        alert("添加成功");
                        tag_table.api().ajax.reload(); //1.10之后
                    } else {
                        alert("添加失败");
                    }
                },
        ...
        // 点击编辑时初始化表单数据
        function tag_OpenModifyForm(tag_id) {
            $.ajax({
                type: "post",
                url: "<?php echo U('blog_tag/getItemById');?>",
                data: {'tag_id': tag_id},
                dataType: "json",
                success: function (data) {
                    // var msg = eval(msg);
//                    console.log(data);
                    if (data.status == 1) {
                        $(".tag-modify-form-modal input[name='tag_id']").val(data.data['tag_id']);
                        $(".tag-modify-form-modal input[name='tag_name']").val(data.data['tag_name']);
                        $(".tag-modify-form-modal input[name='tag_img']").val(data.data['tag_img']);
                        $(".tag-modify-form-modal textarea[name='tag_description']").val(data.data['tag_description']);
                    } else {
                        alert(data.msg);
                    }
                },
        ...
        function tag_Edit() {
            ...
            var jsonData = {
                "tag_id": $(".tag-modify-form-modal input[name='tag_id']").val(),
                "tag_name": $(".tag-modify-form-modal input[name='tag_name']").val(),
                "tag_img": $(".tag-modify-form-modal input[name='tag_img']").val(),
                "tag_description": $(".tag-modify-form-modal textarea[name='tag_description']").val(),
            };
            $.ajax({
                type: "post",
                ...
                success: function (data) {
                    if (data.status == 1) {
                        $('.tag-modify-form-modal').modal('hide');
                        tag_table.api().ajax.reload(null, false); //1.10之后
                    } else {
                        alert(data.msg);
                    }
